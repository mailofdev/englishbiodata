/**
 * Vercel serverless proxy for Vedic Kundli Matching (astrology-api.io).
 * Step 8: Call Gun Milan (Kundli) API. Step 9: In-memory cache (same couple = no upstream call in same instance).
 * POST body: groom/bride with birth_data, or male/female with date (YYYY-MM-DD), time (HH:mm).
 * Set ASTROLOGY_API_KEY in Vercel env (or .env locally).
 */
...
/** Accept birth_data with numeric or string numbers (coerced when building payload). */
...
// Step 9: In-memory cache for same couple (per instance). For persistent cache use Firestore gunMilanCache.
...
export default async function handler(req, res) {
  ...
  // Prefer male/female (date, time) so any proxy or client sending this format is accepted
